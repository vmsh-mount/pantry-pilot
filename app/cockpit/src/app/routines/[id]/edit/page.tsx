  ...
  useEffect(() => {
    api.routines.get(id).then(res => {
      ...
      const r = res.data as {
        ...
        items: { item_name: string; quantity: number; unit: string; swiggy_product_id?: string; swiggy_product_name?: string }[]
      }
      setName(r.name)
  ...
  return (
    <AppShell>
      <div className="flex items-center gap-3 px-1 mb-4">
        <button onClick={() => step === 1 ? router.push(`/routines/${id}`) : setStep(step - 1)}
          className="text-[#D8F3DC] text-sm font-semibold">
          ← Back
        </button>
        <h1 className="text-white font-bold text-lg flex-1">Edit routine</h1>
      </div>
