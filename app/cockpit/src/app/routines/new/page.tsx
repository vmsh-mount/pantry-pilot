...
function Step3({
  ...
  function freqLabel() {
    ...
    if (freqType === "weekly") {
      return `Every ${["Mon","Tue","Wed","Thu","Fri","Sat","Sun"][freqValue]}`
    }
  ...
  return (
    <>
      ...
      <div className="px-6 pb-6 pt-3 border-t border-gray-100 flex gap-3">
        <Button variant="ghost" onClick={onBack}>← Back</Button>
        <Button onClick={onSubmit} loading={loading}>Start routine</Button>
      </div>
    </>
  )
}
...
export default function NewRoutinePage() {
  ...
  async function handleSubmit() {
    ...
    const body = {
      ...
      duration_preset: durationPreset !== "pick_date" ? durationPreset : undefined,
      end_date: durationPreset === "pick_date" ? endDate : undefined,
    }
    const res = await api.routines.create(body)
    setLoading(false)
    if (res.success) {
      router.push("/routines")
    } else {
      setError(res.error?.message ?? "Could not create routine.")
    }
  }
      ...
      <div className="bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col min-h-[420px]">
        ...
        {step === 3 && (
          <Step3
            ...
            onSubmit={handleSubmit} onBack={() => setStep(2)} loading={loading}
          />
        )}
      </div>
    </PageShell>
