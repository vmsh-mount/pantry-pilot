      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          {(() => {
            ...
            // In-progress state is already shown by the main basket card below — suppress chip here
                  ...
                  <div className="px-6 py-8 text-center space-y-3">
                    ...
                    <div>
                      <h2 className="text-base font-semibold text-gray-800">
                        {b?.next_run_at ? `Next basket ${fmtNextRun(b.next_run_at)}` : "Flow is watching"}
                      </h2>
                      <p className="text-sm text-gray-400 mt-1">
                        {b?.next_run_at ? "We'll ping you before anything ships." : "No run scheduled yet."}
                      </p>
                    </div>
                    <button
                      onClick={handleTrigger}
                      disabled={actionLoading === "trigger"}
                      className="text-sm font-medium text-[#2D6A4F] hover:underline disabled:opacity-40"
                    >
                      {actionLoading === "trigger" ? "Starting…" : "Plan now →"}
                    </button>
                  </div>
                </Card>
              )
